# -*- coding: utf-8 -*-
# QEP-07 12da Loader v1.0.1 — quick open to .TIN Mesh (auto-saved), plus options dialog; proper unload()
...
class Qep07LoaderPlugin:
    def __init__(self, iface):
        self.iface = iface
        self.actions = []
        self.menu = "&QEP07 TIN Loader"
        self.toolbar = self.iface.addToolBar("QEP07 TIN Loader")
        self.toolbar.setObjectName("QEP07_TIN_Loader_toolbar")

    def initGui(self):
        icon = QIcon(os.path.join(os.path.dirname(__file__), "icons", "qep07.svg"))
    ...
    def _get_mesh_source_path(self, mesh_layer: QgsMeshLayer):
        try:
            p = mesh_layer.customProperty("QEP07.srcFile", "")
    ...
    def quick_open_tin_sms(self):
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                mesh_layer.setCustomProperty("QEP07.srcFile", out_tin)
            except Exception:
                pass
            QgsProject.instance().addMapLayer(mesh_layer)
        except Exception as e:
